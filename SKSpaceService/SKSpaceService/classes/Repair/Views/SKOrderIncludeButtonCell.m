...
@implementation SKOrderIncludeButtonCell
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.backGroundView addSubview:self.disposeButton];
        [self layoutSubviews];
    }
    return self;
}
...
-(void)layoutSubviews
{
    ...
    [self.disposeButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.serviceTimeLabel.mas_bottom).with.offset(10);
...
{
    if (_disposeButton == nil) {
        _disposeButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [_disposeButton setTitle:@"接单" forState:UIControlStateNormal];
        [_disposeButton setBackgroundColor:UICOLOR_MAIN_ORANGE];
        _disposeButton.layer.cornerRadius = 5.f;
        _disposeButton.layer.borderWidth = 1.f;
        _disposeButton.layer.borderColor = UICOLOR_MAIN_ORANGE.CGColor;
    }
    return _disposeButton;
}
